...
- **v1.1c** — VS Code extension ✅ done — `editors/vscode/` ships a marketplace-ready package: TextMate grammar (keywords, `llm"""` prompts + interpolation, USD literals, `@constraint`s), language configuration, 5 snippets (`llm`/`type`/`par`/`fn`/`test`), and diagnostics wired to `nudgec lsp` over stdio via `vscode-languageclient`; `.vsix` attached to the v1.0.1 release — marketplace publishing only needs a free publisher account
