...
pub fn lex(src: &str) -> Result<Vec<Spanned>, LexError> {
    ...
    while i < b.len() {
        ...
        match c {
                ...
                if word == "llm" && src[i..].starts_with("\"\"\"") {
                    ...
                    match src[i..].find("\"\"\"") {
                        Some(rel) => {
                            ...
                            i += rel + 3;
                            out.push(Spanned { tok: Tok::Prompt(body.to_string()), start: body_start, end: i });
                            continue;
                        }
                        None => return Err(LexError { msg: "unterminated llm\"\"\" prompt".into(), at: start }),
                    }
                }

                let tok = KEYWORDS.iter().find(|(k, _)| *k == word).map(|(_, t)| t.clone())
            ...
            '0'..='9' => {
                ...
                if b.get(i) == Some(&b'.') && b.get(i + 1).map_or(false, |d| d.is_ascii_digit()) {
                    is_float = true; i += 1;
                    while i < b.len() && b[i].is_ascii_digit() { i += 1; }
                }
            ...
            '"' => {
                ...
                loop {
                    match b.get(i) {
                        None => return Err(LexError { msg: "unterminated string".into(), at: start }),
                        Some(&b'"') => { i += 1; break; }
                        Some(&b'\\') => {
                            i += 1;
                            match b.get(i) {
                                ...
                                _ => return Err(LexError { msg: "bad escape".into(), at: i }),
                            }
                            i += 1;
                        }
                        // UTF-8: ASCII fast path, multibyte sequences copied as a slice.
                        Some(&ch) if ch < 0x80 => { s.push(ch as char); i += 1; }
                        Some(&ch) => {
                            let len = if ch >= 0xF0 { 4 } else if ch >= 0xE0 { 3 } else { 2 };
                            if i + len > b.len() {
                                return Err(LexError { msg: "invalid UTF-8 in string".into(), at: i });
                            }
                ...
                out.push(Spanned { tok: Tok::Str(s), start, end: i });
            }
            _ => {
                let start = i;
                ...
                let (tok, len) = match two {
                    "->" => (Tok::Arrow, 2), "|>" => (Tok::Pipe, 2), "+=" => (Tok::PlusEq, 2),
                    "-=" => (Tok::MinusEq, 2), "==" => (Tok::EqEq, 2), "!=" => (Tok::NotEq, 2),
                    "<=" => (Tok::LtEq, 2), ">=" => (Tok::GtEq, 2),
                    _ => match c {
                        '(' => (Tok::LParen, 1), ')' => (Tok::RParen, 1),
                        '{' => (Tok::LBrace, 1), '}' => (Tok::RBrace, 1),
                        '[' => (Tok::LBracket, 1), ']' => (Tok::RBracket, 1),
                        ',' => (Tok::Comma, 1), ':' => (Tok::Colon, 1), '.' => (Tok::Dot, 1),
                        ';' => (Tok::Semi, 1), '@' => (Tok::At, 1), '|' => (Tok::Bar, 1),
                        '=' => (Tok::Assign, 1), '+' => (Tok::Plus, 1), '-' => (Tok::Minus, 1),
                        '*' => (Tok::Star, 1), '/' => (Tok::Slash, 1), '%' => (Tok::Percent, 1),
                        '!' => (Tok::Bang, 1), '<' => (Tok::Lt, 1), '>' => (Tok::Gt, 1),
                        _ => return Err(LexError { msg: format!("unexpected character '{c}'"), at: start }),
                    },
                };
                i += len;
                out.push(Spanned { tok, start, end: i });
    ...
    out.push(Spanned { tok: Tok::Eof, start: i, end: i });
    Ok(out)
}
...
mod tests {
    ...
    fn fn_signature_with_effects() {
        ...
        assert_eq!(t[..8], [
            Tok::Fn, Tok::Ident("research".into()), Tok::LParen,
            Tok::Ident("q".into()), Tok::Colon, Tok::Ident("string".into()),
            Tok::RParen, Tok::Arrow,
        ]);
    ...
    fn par_map_fanout() {
        ...
        assert_eq!(t[..4], [Tok::Par, Tok::Map, Tok::Ident("steps".into()), Tok::Bar]);
    }
    ...
    fn and_or_are_keywords() {
        ...
        assert_eq!(t, vec![
            Tok::Ident("a".into()), Tok::And, Tok::Ident("b".into()),
            Tok::Or, Tok::Ident("c".into()), Tok::Eof,
        ]);
    }
